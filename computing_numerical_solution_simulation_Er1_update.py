...
def ini_T1_belief(beta, beta1, Eb, w_action):
    Er_result=(w_action*(Eb+beta+beta1*Eb)+Eb)/(w_action+(1-beta-beta1*Eb))
...
def main(beta,beta1,Eb2,w_o,sample_size):
    ...
    for re in range(repeat):
        ...
        plt.ylim(0,1)
        plt.ylabel("average belief in T1")
        plt.plot (gen_list,T1_ave_belief_series,color="b")
...
import csv 
df.to_csv ('simulation_result_Er1_update.csv')
